...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  void _showFilterSheet(BuildContext context) {
    showModalBottomSheet(
      context: context, useRootNavigator: true, backgroundColor: AppColors.background,
      isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(14))),
  ...
  // === FIX: Restoring the full implementation of the helper methods ===
          ...
          Consumer(
            builder: (context, ref, child) {
              ...
              return asyncCount.when(
                loading: () => const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2.0, color: AppColors.inactive)),
                error: (err, stack) {
                  // This will print the full error and stack trace to the console
                  print('Error in productCountProvider: $err');
                  print(stack);
                  // And this will show the error message in a tooltip if you tap it
                  return Tooltip(
                    message: err.toString(), // Show the actual error on long-press
                ...
                data: (count) => Container(
                  ...
                  child: Text('${count.filtered}/${count.total}', style: const TextStyle(color: AppColors.inactive, fontSize: 16, fontWeight: FontWeight.w500)),
                ),
              );
            },
          ),
      ...
      children: [
        Expanded(
          child: TextButton.icon(
            ...
            style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 8.0), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8.0), side: BorderSide(color: AppColors.inactive.withAlpha(128)))),
          ),
        ),
...
// === FIX: Restoring the full implementation of the _GroupHeader class ===
class _GroupHeader extends StatelessWidget {
  const _GroupHeader({super.key, required this.style});
